...
@DataMongoTest(includeFilters = @ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE,
  classes = {MongoConfig.class, BatchRepositoryImpl.class, OrderLineRepositoryImpl.class,
    BatchMapperImpl.class, OrderLineMapperImpl.class}))
public class AllocationRepositoriesTestIT {
  ...
  public void testWhenInsert3Then3AreExpected() {
    StepVerifier.create(
        orderLineRepository.deleteAll()
          .thenMany(
            orderLineRepository.saveAll(Flux.just(
              OrderLine.builder().sku("GENERIC-SOFA").quantity(12).orderId("order1").build(),
              OrderLine.builder().sku("RED-TABLE").quantity(13).orderId("order1").build(),
              OrderLine.builder().sku("GENERIC-SOFA").quantity(14).orderId("order2").build()
            ))
          ))
      .expectNextCount(3)
      .verifyComplete();
  ...
  public void testRepositoryCanRetrieveABatchWithAllocations() {
    ...
    batchRepository.findByReference("batch1")
      .as(StepVerifier::create)
      .assertNext(savedBatch -> {
